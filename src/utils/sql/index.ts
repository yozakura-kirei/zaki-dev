...
//     ORDER BY MAX(iac.created_at) DESC \
//     LIMIT 50;',
//   // お知らせを取得
//   getNotices:
//     'SELECT \
...
//     ta.content, \
//     ta.is_fixed, \
//     GROUP_CONCAT(mc.name) AS categories, \
//     GROUP_CONCAT(mc.search_name) AS search_name, \
//     ta.created_at, \
//     ta.updated_at  \
//     FROM i_articles_categories iac \
...
//   // 記事の詳細を取得
//   getArticleId: `SELECT \
//     ta.id, ta.article_id, ta.title, ta.content, ta.is_fixed, GROUP_CONCAT(mc.name) AS categories, GROUP_CONCAT(mc.search_name) AS search_name, ta.created_at, ta.updated_at \
//     FROM i_articles_categories iac \
//     LEFT JOIN t_articles ta ON ta.id = iac.t_articles_id \
//     LEFT JOIN m_categories mc ON mc.id = iac.m_categories_id \
//     WHERE ta.article_id = ? \
...
//     LIMIT ?;`,
//   // articleIdのみを取得
//   getOnlyArticleId: `SELECT ta.frticle_id FROM t_articles ta LIMIT ?;`,
//   // noticeIdのみを取得
//   getOnlyNoticeId: `SELECT tn.notice_id FROM t_notices tn LIMIT ?;`,
//   // categoryのsearch_nameのみを取得
//   getOnlyCategorySearchName: `SELECT mc.search_name FROM m_categories mc WHERE mc.name IS NOT NULL AND mc.name != 'カテゴリなし' LIMIT ?;`,
//   // categoryのcategory_nameのみを取得
//   getOnlyCategoryName: `SELECT mc.name FROM m_categories mc WHERE mc.search_name LIKE ? LIMIT ?;`,
//   /** 検索 */
//   // 記事
